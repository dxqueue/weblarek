...
Этот класс управляет всем, что связано с товарами: хранит их полный список, запоминает, какой товар выбран для просмотра, и дает методы для работы с каталогом.

Конструктор класса не принимает параметров.

Поля класса:
`catalog: IProduct[]` - хранит каталог товаров, полученных с сервера
`cardProduct: IProduct` - хранит карточку выбранного товара из каталога
